...
type t=Ratio of Big_int.big_int*Big_int.big_int;;
 ...
 let denominator (Ratio(a,b))=Big.to_string b;;
  ...
  let dot i (Ratio(a,b))=
    direct_frac (Big.mult (Big.of_int i) a) b;;
  ...
  let sign r=if is_negative(r) then '-' else 
             if is_zero(r) then '0' else '+';; 	
             
  let is_nonzero r=not(is_zero r);;           
  ...
  let minimize f =function
  		[]->failwith("nothing to minimize")
  		|x1::b->
   			let y1=f(x1) in
   			let rec tempf=
   				(function  ((x,y),da_ober)->
     					match da_ober with
	 					[]->(x,y)
	 					|x2::peurrest->let y2=f(x2) in
	 								   if compare(y2)(y)
  ...
  let min x y=if  gt x y then y else x;;
 ...
 let inverse x=div one x;;
  ...
  let nearest_integer (Ratio(a,b))=
   ...
   let insider (Ratio(a,b)) (Ratio(c,d))=
     direct_frac (Big.add a c) (Big.add b d);;
   
  let to_int rat=
    match nearest_integer(rat) with
    Ratio(a,b)->Big_int.int_of_big_int a;;
  
  let ten=frac 10 1;;
 
  let rec lower_depth x=
    let rec tempf=(fun (x,j)->
     if leq  one x then j else
     tempf(mult x ten,j+1)
    ) in
    tempf(max x (opposite x),0);;
  
   let rec upper_depth x=
    let rec tempf=(fun (x,j)->
     if leq  x one  then j else
     tempf(div x ten,j+1)
    ) in
    tempf(max x (opposite x),0);;
